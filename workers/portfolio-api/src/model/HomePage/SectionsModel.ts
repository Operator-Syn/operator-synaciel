// workers/portfolio-api/src/model/HomePage/SectionsModel.ts
...
export class SectionsModel {
  ...
  async list() {
    const res = await this.db
      .prepare(
        "SELECT id, title, section_type, display_order FROM sections ORDER BY display_order ASC, id ASC",
      )
      .all<SectionRow>();
    return res.results;
  }
  ...
  async update(id: number, title: string, type: string, display_order: number) {
    return this.db
      ...
      .bind(title, type, display_order, id)
      .first<SectionRow>();
  }
