...
export function registerRepositoryTools(server: McpServer): void {
  server.registerTool(
    'prepare_repository_change',
    {
      ...
      description: 'Read-only preparation for a bounded text-file change. Returns exact paths, hashes, a reviewable diff, and a one-time apply token without modifying the checkout.',
      inputSchema: {
  ...
  server.registerTool(
    'apply_repository_change',
    {
      ...
      description: 'Approval-gated local apply. Requires the exact one-time token, explicit approval, and a hash map covering exactly the prepared files. Rolls back on failure.',
      inputSchema: {
  ...
  server.registerTool(
    'verify_repository_change',
    {
      ...
      description: 'Runs only fixed repository-native npm checks selected by a named verification profile. Arbitrary shell commands and deployment operations are not accepted.',
      inputSchema: {
  ...
  server.registerTool(
    'prepare_working_tree_commit',
    {
      ...
      description: 'Read-only snapshot of every visible dirty path, including untracked files, deletions, binary hashes, and a reviewable diff. Restricted developer paths require one-time explicit consent.',
      inputSchema: {
  ...
  server.registerTool(
    'git_commit_working_tree',
    {
      ...
      description: 'Approval-gated local commit of the exact prepared snapshot. Rechecks hashes and status, requires one reviewed sentence-style subject per file, keeps hooks active, and reports partial progress.',
      inputSchema: {
    ...
    async (input) => result(await commitWorkingTree(input.operationId, input.approvalHash, input.commits as CommitEntry[])),
  );
  ...
  server.registerTool(
    'prepare_commits',
    {
      ...
      description: 'Read-only helper that rechecks applied file hashes and suggests one subject per file. The reviewer must edit the subjects before approval.',
      inputSchema: {
        operationId: z.string().min(1),
        approvalHash: z.string().length(64),
      },
  ...
  server.registerTool(
    'git_commit_files',
    {
      ...
      description: 'Approval-gated local commit for an already-applied change. Requires exact file coverage, rejects unrelated or stale changes, keeps hooks active, and creates one commit per reviewed file.',
      inputSchema: {
        operationId: z.string().min(1),
        approvalHash: z.string().length(64),
        commits: commitEntries,
    ...
    async (input) => result(await commitAppliedFiles(input.operationId, input.approvalHash, input.commits as CommitEntry[])),
  );
}
