...
describe('approval-gated commit pipeline', () => {
  test('commits a reviewed working-tree path through the real pre-commit hook', async () => {
    ...
    await writeFile(join(repository, 'src/one.ts'), 'one after\n');
    const server = await startServer(repository);
    servers.push(server);

    const prepared = payload(await server.call('tools/call', {
      name: 'prepare_working_tree_commit',
      arguments: {},
    }));
    assert.equal(prepared.status, 'prepared');
    assert.deepEqual(prepared.paths, ['src/one.ts']);

    const committed = payload(await server.call('tools/call', {
      name: 'git_commit_working_tree',
      arguments: {
        operationId: prepared.operationId,
        approvalHash: prepared.approvalHash,
        commits: [{ path: 'src/one.ts', message: 'Update the fixture source behavior.' }],
      },
    }));
    assert.equal(committed.status, 'committed');
    assert.deepEqual(committed.filesPerCommit, [1]);
    assert.equal(runGit(repository, ['status', '--short']).stdout.trim(), '');
    assert.equal(requireGit(repository, ['diff-tree', '--no-commit-id', '--name-only', '-r', 'HEAD']), 'src/one.ts');
  });
  ...
  test('requires one-time consent for restricted paths without exposing the marker', async () => {
    ...
    const challenge = payload(await server.call('tools/call', {
      name: 'prepare_working_tree_commit',
      arguments: {},
    }));
    assert.equal(challenge.status, 'consent_required');
    assert.equal(JSON.stringify(challenge).includes('OPERATOR_SYNACIEL_COMMIT_PIPELINE_APPROVAL'), false);
  ...
  test('applies a hashed change and commits the exact applied file', async () => {
    ...
    const prepared = payload(await server.call('tools/call', {
      ...
      arguments: {
        ...
        profile: 'app',
        operations: [{ path: 'src/one.ts', content: 'one applied\n', expectedSha256 }],
      },
    }));
    assert.equal(prepared.status, 'prepared');

    const applied = payload(await server.call('tools/call', {
    ...
    const suggestions = payload(await server.call('tools/call', {
      name: 'prepare_commits',
      arguments: { operationId: applied.operationId, approvalHash: applied.approvalHash },
    }));
    assert.deepEqual(suggestions.paths, ['src/one.ts']);
    ...
    const committed = payload(await server.call('tools/call', {
      ...
      arguments: {
        ...
        commits: [{ path: 'src/one.ts', message: 'Apply the reviewed source change.' }],
      },
    }));
    assert.equal(committed.status, 'committed');
    assert.equal(runGit(repository, ['status', '--short']).stdout.trim(), '');
  ...
  test('rejects duplicate, unrelated, stale, and path-only commit requests before mutation', async () => {
    const repository = await createRepository();
    repositories.push(repository);
    await writeFile(join(repository, 'src/one.ts'), 'one changed\n');
    await writeFile(join(repository, 'src/two.ts'), 'two changed\n');
    const server = await startServer(repository);
    servers.push(server);
  ...
  test('rolls back earlier writes when a later prepared write cannot be applied', async () => {
    const repository = await createRepository();
    repositories.push(repository);
    await writeFile(join(repository, 'src/not-dir'), 'this is a file\n');
    const server = await startServer(repository);
    servers.push(server);
    ...
    const prepared = payload(await server.call('tools/call', {
      ...
      arguments: {
        ...
        profile: 'app',
        operations: [
          { path: 'src/created.ts', content: 'created then rolled back\n' },
          { path: 'src/not-dir/blocked.ts', content: 'this cannot be written\n' },
        ],
      },
    }));
    assert.equal(prepared.status, 'prepared');
    ...
    await assert.rejects(() => access(join(repository, 'src/created.ts')));
  });
  ...
  test('commits untracked binary files and deletions as separate hook-visible commits', async () => {
    ...
    await writeFile(join(repository, 'public/fixture.png'), Buffer.from([0, 1, 2, 3]));
    await rm(join(repository, 'src/one.ts'));
    const server = await startServer(repository);
    servers.push(server);
    ...
    const paths = prepared.paths as string[];
    assert.deepEqual(new Set(paths), new Set(['public/fixture.png', 'src/one.ts']));
    const committed = payload(await server.call('tools/call', {
      name: 'git_commit_working_tree',
      arguments: {
        operationId: prepared.operationId,
  ...
  test('the versioned pre-commit hook rejects direct multi-path commits', async () => {
    const repository = await createRepository();
    repositories.push(repository);
    await writeFile(join(repository, 'src/one.ts'), 'one direct\n');
    await writeFile(join(repository, 'src/two.ts'), 'two direct\n');
