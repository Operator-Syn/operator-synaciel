...
export const serverEntry = resolve(repositoryRoot, "tools/repository-mcp/src/server.ts");
...
export async function createRepository(): Promise<string> {
  ...
  await writeFile(join(repository, "apps/portfolio-web/src/one.ts"), "one before\n");
