...
function resolveNewPushBase(cwd: string, remoteRef?: string): string | null {
  ...
  if (remoteRef?.startsWith("refs/heads/"))
    candidateRefs.add(`origin/${remoteRef.slice("refs/heads/".length)}`);
...
export function resolveAuditRange(options: HistoryAuditOptions): string | null {
  ...
  if (options.remoteSha && !isZeroSha(options.remoteSha))
    return `${options.remoteSha}..${options.localSha}`;
...
function commitParents(cwd: string, hash: string): readonly string[] {
  return requireGit(cwd, ["rev-list", "--parents", "-n", "1", hash])
    .stdout.trim()
    .split(/\s+/)
    .filter(Boolean)
    .slice(1);
...
export function auditCommitRange(options: {
  readonly cwd?: string;
  readonly range: string;
}): HistoryAuditResult {
  const cwd = options.cwd ?? process.cwd();
  const commits = requireGit(cwd, ["rev-list", "--reverse", options.range])
    .stdout.trim()
    .split(/\s+/)
    .filter(Boolean);
  ...
  for (const hash of commits) {
    ...
    if (paths.length === 0) violations.push({ hash, subject, paths, reason: "empty" });
      ...
      violations.push({ hash, subject, paths, reason: "multiple-paths" });
  }
...
export function formatAuditFailure(result: HistoryAuditResult): string {
  const lines = [
    "One-file commit audit failed.",
    ...(result.range ? [`Audited range: ${result.range}`] : []),
  ];
  for (const violation of result.violations) {
    const detail =
      violation.reason === "merge"
        ? "merge commit"
